...
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(A2AMesonFieldMILCPar, int, block, std::string,
...
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(A2AMesonFieldMILCMetadata, std::vector<RealF>,
                                  momentum, StagGamma::StagAlgebra, gamma_spin,
                                  StagGamma::StagAlgebra, gamma_taste);
};

template <typename T, typename FImpl>
class MesonFieldKernelMILCOld
    : public A2AKernelMILC<T, typename FImpl::FermionField> {
public:
  FERM_TYPE_ALIASES(FImpl, );

public:
  MesonFieldKernelMILCOld(GridBase *grid,
...
public:
  ...
  typedef A2AMatrixBlockComputationMILC<
      Complex, FermionField, A2AMesonFieldMILCMetadata, HADRONS_A2AM_IO_TYPE>
      Computation;
...
MODULE_REGISTER(StagA2AMesonFieldOld,
                ARG(TA2AMesonFieldMILC<STAGIMPL, MassShiftEigenPack<STAGIMPL>>),
                MContraction);
...
TA2AMesonFieldMILC<FImpl, Pack>::TA2AMesonFieldMILC(const std::string name)
    : Module<A2AMesonFieldMILCPar>(name), _momphName(name + "_momph") {}
...
void TA2AMesonFieldMILC<FImpl, Pack>::setup(void) {
  ...
  envCache(std::vector<ComplexField>, _momphName, 1, par().mom.size(),
...
void TA2AMesonFieldMILC<FImpl, Pack>::execute(void) {
  ...
  if (hasLowModes) {
    ...
    if (isCheckerBoarded) {
      ...
      computation.execute(*left, *right, kernel, ionameFn, filenameFn,
                          metadataFn, &lowModes.evec, lowModes.eval,
                          &swapEvecCheckerFn);
    } else {
      Kernel kernel(envGetGrid(FermionField), _gammas, ph, U);
      computation.execute(*left, *right, kernel, ionameFn, filenameFn,
