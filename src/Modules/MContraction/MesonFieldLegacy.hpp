/*
 * MesonFieldLegacy.hpp, part of Hadrons (https://github.com/aportelli/Hadrons)
 *
...
/*  END LEGAL */
#ifndef HadronsMILC_MContraction_A2AMesonFieldLegacy_hpp_
#define HadronsMILC_MContraction_A2AMesonFieldLegacy_hpp_
...
#include <A2AMatrix.hpp> // [Phase 4 repoint: <A2AMatrixLegacy.hpp>]
#include <GridMilc/GridMilc.h>

BEGIN_HADRONS_NAMESPACE

/******************************************************************************
 *        All-to-all meson field creation (legacy 4-field worker path)       *
 ******************************************************************************/
BEGIN_MODULE_NAMESPACE(MContraction)

// Legacy flavour of the A2A meson-field module: the pre-stencil
// A2AWorkerLocal/Onelink/SpinTaste workers behind the 4-field (left/right x
// even/odd) kernel, with checkerboard-evec swapping via Meooe, gamma
// popcount group splitting, and the coefficient bake (norm/eigenvalue
// pre-multiplication) at write time. The canonical StagA2AMesonField
// registration moved to the stencil implementation in MesonField.hpp; this
// module registers StagA2AMesonFieldLegacy. All class names here are
// pairwise distinct from the stencil module's: both headers compile into
// one TU via the auto-generated Modules.hpp.
class MesonFieldLegacyMILCPar : Serializable {
public:
...
class MesonFieldKernelMILCLegacy
    : public A2AKernelMILC<T, typename FImpl::FermionField> { // [Phase 4 repoint: A2AKernelMILCLegacy]
public:
  FERM_TYPE_ALIASES(FImpl, );

public:
  MesonFieldKernelMILCLegacy(GridBase *grid) {
    _vol = 1.;
    for (auto &d : grid->GlobalDimensions()) {
      _vol *= d;
    }
  }
  virtual ~MesonFieldKernelMILCLegacy(void) {};
...
class TMesonFieldMILCLegacy : public Module<MesonFieldLegacyMILCPar> {
public:
  FERM_TYPE_ALIASES(FImpl, );
  typedef A2AMatrixBlockComputationMILC< // [Phase 4 repoint: A2AMatrixBlockComputationMILCLegacy]
      Complex, FermionField, MesonFieldLegacyMILCMetadata,
      HADRONS_A2AM_IO_TYPE>
      Computation;
  typedef MesonFieldKernelMILCLegacy<Complex, FImpl> Kernel;

public:
  // constructor
  TMesonFieldMILCLegacy(const std::string name);
  // destructor
  virtual ~TMesonFieldMILCLegacy(void) {};
...
MODULE_REGISTER(
    ...
    ARG(TMesonFieldMILCLegacy<STAGIMPL, MassShiftEigenPack<STAGIMPL>>),
    MContraction);

/******************************************************************************
 *                  TMesonFieldMILCLegacy implementation                     *
 ******************************************************************************/
// constructor /////////////////////////////////////////////////////////////////
template <typename FImpl, typename Pack>
TMesonFieldMILCLegacy<FImpl, Pack>::TMesonFieldMILCLegacy(
    const std::string name)
    : Module<MesonFieldLegacyMILCPar>(name), _momphName(name + "_momph") {}

// dependencies/products ///////////////////////////////////////////////////////
template <typename FImpl, typename Pack>
std::vector<std::string> TMesonFieldMILCLegacy<FImpl, Pack>::getInput(void) {
...
template <typename FImpl, typename Pack>
std::vector<std::string> TMesonFieldMILCLegacy<FImpl, Pack>::getOutput(void) {
...
// execution ///////////////////////////////////////////////////////////////////
template <typename FImpl, typename Pack>
void TMesonFieldMILCLegacy<FImpl, Pack>::execute(void) {
  ...
  auto gammaMetadataFn = [this](const unsigned int m, const unsigned int g) {
    MesonFieldLegacyMILCMetadata md;
...
#endif // HadronsMILC_MContraction_A2AMesonFieldLegacy_hpp_
