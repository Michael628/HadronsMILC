...
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(ASlashMesonFieldMILCMetadata,
                                  std::vector<RealF>, momentum, std::string,
                                  amu_seed, int, amu_index);

  ASlashMesonFieldMILCMetadata() : momentum{}, amu_index(-1), amu_seed("N/A") {}
};

template <typename T, typename FImpl>
class ASlashMesonFieldKernelMILC
    : public A2AKernelMILC<T, typename FImpl::FermionField> {
public:
  FERM_TYPE_ALIASES(FImpl, );

public:
  ASlashMesonFieldKernelMILC(GridBase *grid,
                             const std::vector<ComplexField> &mom,
                             const std::vector<ComplexField> &Amu,
                             int orthogDir) {
    _worker =
        std::make_unique<A2AWorkerLocal<FImpl>>(grid, mom, Amu, orthogDir);
    _vol = 1.;
...
public:
  ...
  typedef A2AMatrixBlockComputationMILC<
      Complex, FermionField, ASlashMesonFieldMILCMetadata, HADRONS_A2AM_IO_TYPE>
      Computation;
...
MODULE_REGISTER(
    StagA2AASlashMesonField,
    ARG(TASlashMesonFieldMILC<STAGIMPL, MassShiftEigenPack<STAGIMPL>>),
    MContraction);
...
// constructor /////////////////////////////////////////////////////////////////
template <typename FImpl, typename Pack>
TASlashMesonFieldMILC<FImpl, Pack>::TASlashMesonFieldMILC(
    const std::string name)
    : Module<ASlashMesonFieldMILCPar>(name), _momphName(name + "_momph"),
      _EmName(name + "_em") {}
...
void TASlashMesonFieldMILC<FImpl, Pack>::setup(void) {
  ...
  envCache(std::vector<ComplexField>, _EmName, 1, par().nEmFields,
           envGetGrid(ComplexField));
...
void TASlashMesonFieldMILC<FImpl, Pack>::execute(void) {
  ...
  auto AmuFilenameFn = [this, &AmuIOnameFn](const unsigned int m,
  ...
  auto AmuMetadataFn = [this](const unsigned int m, const unsigned int g) {
    ...
    md.amu_seed = par().EmSeedString.empty()
                      ? "<blank> see output file for seed string!"
                      : par().EmSeedString;
    md.amu_index = g;
  ...
  if (hasLowModes) {
    ...
    if (isCheckerBoarded) {
      ...
      if (par().nEmFields > 0) {
        ...
        computation.execute(*left, *right, kernel, AmuIOnameFn, AmuFilenameFn,
                            AmuMetadataFn, &lowModes.evec, lowModes.eval,
                            &swapEvecCheckerFn);
      }
    } else {
      if (par().nEmFields > 0) {
