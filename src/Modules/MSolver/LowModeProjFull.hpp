/*
 * LowModeProjMILCFull.hpp, part of Hadrons
...
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(LowModeProjMILCFullPar, std::string, action,
                                  bool, projector, std::string, lowModes);
};
...
MODULE_REGISTER_TMP(
    StagLMAFull,
    ARG(TLowModeProjMILCFull<STAGIMPL, MassShiftEigenPack<STAGIMPL>>), MSolver);
...
void TLowModeProjMILCFull<FImpl, Pack>::setup(void) {
  ...
  auto makeSolver = [&mat, &epack, project, this](bool subGuess) {
    return [&mat, &epack, subGuess, project, this](FermionField &sol,
      ...
      for (int k = epack.evec.size() - 1; k >= 0; k--) {
        ...
        const ComplexD ipDag =
            TensorRemove(innerProduct(tempDag, source)) * conjugate(lam);
