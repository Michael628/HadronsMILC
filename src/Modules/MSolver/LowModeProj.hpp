...
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(LowModeProjMILCPar, std::string, action, bool,
                                  projector, unsigned int, eigStart, int, nEigs,
                                  std::string, lowModes);
};
...
MODULE_REGISTER_TMP(
    StagLMA, ARG(TLowModeProjMILC<STAGIMPL, MassShiftEigenPack<STAGIMPL>>),
    MSolver);
...
void TLowModeProjMILC<FImpl, Pack>::setup(void) {
  ...
  auto makeSolver = [&mat, &epack, project, eigStart, nEigs,
                     this](bool subGuess) {
    return [&mat, &epack, subGuess, project, eigStart, nEigs,
            this](FermionField &sol, const FermionField &source) {
      ...
      for (int k = (eigStart + nEigs - 1); k >= int(eigStart); k--) {
        ...
        if (!project) {
          ...
          axpy(rbTempNeg, mass * ipNeg * invlam_D * invlam_D - ip, e,
               rbTempNeg);
        } else {
