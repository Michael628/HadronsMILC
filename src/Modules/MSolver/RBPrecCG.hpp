...
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(RBPrecCGMILCPar, std::string, action,
                                  unsigned int, maxIteration, double, residual,
                                  std::string, isEven, std::string, guesser);
};

template <typename FImpl> class TRBPrecCGMILC : public Module<RBPrecCGMILCPar> {
public:
...
TRBPrecCGMILC<FImpl>::TRBPrecCGMILC(const std::string name) : Module(name) {}
...
template <typename FImpl> void TRBPrecCGMILC<FImpl>::setup(void) {
  ...
  if (!par().isEven.empty()) {
    if (!(std::istringstream(par().isEven) >> std::boolalpha >> isEven_)) {
      HADRONS_ERROR(Logic, "parameter isEven='" + par().isEven +
  ...
  auto makeSolver = [&mat, guesserPt, this](bool subGuess) {
    return [&mat, guesserPt, subGuess,
            this](FermionField &sol, const FermionField &source) { assert(0); };
  };

  auto makeGuessSolver = [&mat, guesserPt, this](bool subGuess) {
    return [&mat, subGuess, guesserPt, this](FermionField &sol,
                                             const FermionField &source,
                                             const FermionField &guess) {
      conformable(source, guess);
      // ZeroGuesser<FermionField>    defaultGuesser;
      // LinearFunction<FermionField> &guesser = (guesserPt == nullptr) ?
      // defaultGuesser : *guesserPt;
...

      {
        ...
        guesser = new GuessWrapper(guessRb);
      }
    ...
    FermionField solRb(envGetRbGrid(FermionField)),
        solRbNeg(envGetRbGrid(FermionField));
    LOG(Message) << "Improving residual of complementary checkerboard."
                 << std::endl;
    ...
    LOG(Message) << "Final combined true residual: " << std::sqrt(nr / ns)
                 << std::endl;
#endif
      LOG(Message) << "Improving residual with unpreconditioned solver."
                   << std::endl;
      MdagMLinearOperator<FMat, FermionField> hermOp(mat);

      mat.Mdag(source, test);
      cg(hermOp, test, sol);
      mat.M(sol, test);
      test = test - source;
      RealD ns = norm2(source);
      RealD nr = norm2(test);
      // LOG(Message) << "..........Combining checkerboards." << std::endl;
      LOG(Message) << "Final true residual: " << std::sqrt(nr / ns)
                   << std::endl;
      if (subGuess) {
        sol = sol - guess;
      }
...
// execution ///////////////////////////////////////////////////////////////////
template <typename FImpl> void TRBPrecCGMILC<FImpl>::execute(void) {}
