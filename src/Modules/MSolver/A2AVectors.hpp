...
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(A2AVectorsMILCPar, std::string, noise,
                                  std::string, action, std::string, lowModes,
                                  std::string, solver, std::string, highOutput,
                                  bool, highMultiFile);
};
...
void TA2AVectorsMILC<FImpl, Pack>::setup(void) {
  ...
  envCreate(std::vector<FermionField>, getName() + "_w", 1, Nh_,
  ...
  envCreate(std::vector<FermionField>, getName() + "_v", 1, Nh_,
            envGetGrid(FermionField));
...
void TA2AVectorsMILC<FImpl, Pack>::execute(void) {
  ...
  if (hasLow) {
    LOG(Message) << "Computing all-to-all vectors "
                 << " using lowModes '" << par().lowModes
                 << "' (low modes) and noise '" << par().noise << "' (" << Nh_
  ...
  if (hasLow && !lowGuess) {
    LOG(Message)
        << "Projecting low contribution from stochastic high mode sources"
        << std::endl;
  ...
  for (int ih = 0; ih < Nh_; ih++) {
    ...
    if (lowGuess) {
      ...
      LOG(Message) << "check orthog <v,guess>: "
                   << TensorRemove(innerProduct(v[ih], g)) * n_temp
                   << std::endl;
      n_temp = 1.0 / ::sqrt(norm2(temp) * norm2(w[ih]));
      LOG(Message) << "check orthog: <w,proj>:  "
                   << TensorRemove(innerProduct(w[ih], temp)) * n_temp
                   << std::endl;
    } else {
  ...
  if (!par().highOutput.empty()) {
    startTimer("V I/O");
    A2AVectorsIo::write(par().highOutput + "_v", v, par().highMultiFile,
                        vm().getTrajectory());
    stopTimer("V I/O");
    startTimer("W I/O");
    A2AVectorsIo::write(par().highOutput + "_w", w, par().highMultiFile,
                        vm().getTrajectory());
    stopTimer("W I/O");
  }
}
