...
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(TrajRange, unsigned int, start, unsigned int,
                                  end, unsigned int, step);
};

class GlobalPar : Serializable {
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(GlobalPar, TrajRange, trajCounter,
                                  unsigned int, nt, std::string, diskVectorDir,
                                  std::string, output);
};

class EpackPar : Serializable {
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(EpackPar, std::string, fileStem, bool,
                                  evalFile, bool, multiFile, int, nEigs, RealD,
                                  massOld, RealD, massNew);
  EpackPar(void)
      : fileStem{"N/A"}, evalFile{false}, multiFile{false}, nEigs{0},
        massOld{0.0}, massNew{0.0} {}
};

class EigenValueFile : Serializable {
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(EigenValueFile, std::vector<RealD>, evals);
};

class A2AMatrixPar : Serializable {
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(A2AMatrixPar, std::string, file, std::string,
                                  dataset, unsigned int, cacheSize,
                                  unsigned int, ni, unsigned int, nj,
                                  unsigned int, niOffset, unsigned int,
                                  njOffset, std::string, name, EpackPar, epack);
};

class ProductPar : Serializable {
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(ProductPar, std::string, terms,
                                  std::vector<std::string>, times, std::string,
                                  translations, bool, translationAverage, bool,
                                  spaceNormalize);
};
...
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(CorrelatorResult,
                                  std::vector<ContractorMILC::A2AMatrixPar>,
                                  a2aMatrix, ProductPar, contraction,
...
void makeTimeSeq(std::vector<std::vector<unsigned int>> &timeSeq,
                 ...
                 std::vector<unsigned int> &current, const unsigned int depth) {
  if (depth > 0) {
...
void saveCorrelator(const ContractorMILC::CorrelatorResult &result,
                    const std::string dir, const unsigned int dt,
                    const unsigned int traj) {
  std::string fileStem = "", filename;
  std::vector<std::string> terms =
      strToVec<std::string>(result.contraction.terms);
...
std::set<unsigned int> parseTimeRange(const std::string str,
  ...
  for (auto &s : rstr) {
    ...
    if (sm[1].matched) {
      ...
      if (t >= nt) {
        HADRONS_ERROR(Range,
                      "time out of range (from expression '" + str + "')");
      }
      tSet.insert(t);
...
struct Sec {
  Sec(const double usec) { seconds = usec / 1.0e6; }
...
int main(int argc, char *argv[]) {
  ...
  for (auto &p : par.a2aMatrix) {
    ...
    a2aMat.emplace(
        p.name, EigenDiskVector<ComplexD>(dirName, par.global.nt, p.cacheSize));
  }
  ...
  for (unsigned int traj = par.global.trajCounter.start;
    ...
    for (auto &p : par.a2aMatrix) {
      ...
      A2AMatrixIoMILC<HADRONS_A2AM_IO_TYPE> a2aIo(filename, p.dataset,
                                                  par.global.nt, p.ni, p.nj,
                                                  p.niOffset, p.njOffset);

      a2aIo.load(a2aMat.at(p.name), &t, grid);
      LOG(Message) << "Read " << a2aIo.getSize() << " bytes in " << t / 1.0e6
                   << " sec, " << a2aIo.getSize() / t * 1.0e6 / 1024 / 1024
                   << " MB/s" << std::endl;
      ...
      if (!p.epack.fileStem.empty()) {
        ...
        if (p.epack.evalFile) {
          ResultReader reader(
              ModuleBase::resultFilename(p.epack.fileStem, traj));
          read(reader, "evals", evals);
        } else {
          PackRecord record;
          std::string filename = p.epack.fileStem + "." + std::to_string(traj) +
                                 (p.epack.multiFile ? "" : ".bin");
        ...
        for (int i = nEvals; i > p.njOffset / 2; i--) {
          if (massOld == massNew) {
            if (i == nEvals) {
              LOG(Message) << "Removing  Eigenvalue weight from " << p.name
                           << " with mass=" << massNew << std::endl;
            }
            newEval = ComplexD(2.0 * p.epack.massOld, sqrt(evals[i - 1]));
          } else {
            if (i == nEvals) {
              LOG(Message) << "Shifting Eigenvalues of " << p.name
                           << " from mass=" << massOld << " to mass=" << massNew
                           << std::endl;
            }
            newEval = ComplexD(2.0 * p.epack.massOld, sqrt(evals[i - 1])) /
                      ComplexD(2.0 * p.epack.massNew, sqrt(evals[i - 1]));
          }
    ...
    if (grid->IsBoss()) {
      for (auto &p : par.product) {
        ...
        if (term.size() != p.times.size() + 1) {
          HADRONS_ERROR(Size, "number of terms (" +
        ...
        for (auto &m : par.a2aMatrix) {
          if (std::find(result.a2aMatrix.begin(), result.a2aMatrix.end(), m) ==
              result.a2aMatrix.end()) {
        ...
        LOG(Message) << timeSeq.size() * translations.size() * (term.size() - 2)
                     << " A*B, "
                     << timeSeq.size() * translations.size() * par.global.nt
                     << " tr(A*B)" << std::endl;
        ...
        for (unsigned int t = 0; t < par.global.nt; ++t) {
          ...
          thread_for(j, ref.cols(), {
            for (unsigned int i = 0; i < ref.rows(); ++i) {
              if (evalMap.count(term.back()) > 0 &&
                  j < evalMap.at(term.back()).size()) {
        ...
        LOG(Message) << Sec(tAr.getDTimer("Transpose caching")) << " "
                     << Bytes(bytes, tAr.getDTimer("Transpose caching"))
                     << std::endl;
          ...
          for (auto &dt : translations) {
            ...
            if (term.size() > 2) {
              std::cout << Sec(tAr.getDTimer("A*B total") - busec) << " "
                        ...
                        << " "
                        << Bytes(bytes, tAr.getDTimer("A*B total") - busec)
                        << std::endl;
            }
            ...
            std::cout << Sec(tAr.getDTimer("tr(A*B)") - busec) << " "
                      ...
                      << Bytes(bytes, tAr.getDTimer("tr(A*B)") - busec)
                      << std::endl;
