...
def build_params(**module_templates):
    ...
    for s in range(int(env["NSEEDS"])):
        ...
        module["id"]["name"] = f"a2avec_{seed_string}"
        module["options"]["noise"] = f"noise_{seed_string}"
        module["options"]["action"] = f"stag_{mass_string}"
        module["options"]["lowModes"] = quark
        module["options"]["solver"] = f"stag_ama_{mass_string}"
        module["options"]["highOutput"] = f"e{env['EIGS']}n{env['NOISE']}dt{env['DT']}/proj_vectors/{mass_string}/{seed_string}"
