...
def time_average(cij,average=True,open_indices=(0,-1)):
    ...
    corr = cpnp.zeros(cij[time_removed_indices].shape+(nt,),dtype=np.complex128)

    for t in range(nt):
        corr[...,t] = cij[t_mask==t].sum()
...
class MesonLoader:
    ...
    shift_mass : bool, optional
        ...
        from eigenvectors with eigenvalues based on `oldmass`. The eigenvalues will
        then be replaced by corresponding values using `newmass` via the `meson_mass_alter` method.
    evalfile : str, optional
        File location of hdf5 file containing list of eigenvalues. Required if `shift_mass` is True.
    ...
    def meson_mass_alter(self,mat: cpnp.ndarray):
        ...
        with h5py.File(self.evalfile,'r') as f:
            try:
                evals = cpnp.array(f['/evals'][()].view(cpnp.float64),dtype=np.float64)
            except KeyError:
                evals = cpnp.array(f['/EigenValueFile/evals'][()].view(cpnp.float64),dtype=cpnp.float64)
        ...
        mat[:] = cpnp.multiply(mat,eval_scaling[cpnp.newaxis,cpnp.newaxis,:])
    ...
    def load_meson(self, file, time: slice=slice(None)):
        ...
        t2 = perf_counter()
        logging.debug(f"Loaded array {temp.shape} in {t2} sec")

        if self.shift_mass:
            fact = "2*" if self.milc_mass else ""
            logging.info(f"Shifting mass from {fact}{self.oldmass:f} to {fact}{self.newmass:f}")
...
class Contractor:
    ...
    def __init__(self, series: str, cfg: str, **kwargs):
        ...
        if 'newmass' in self.__dict__:
            ...
            self.meson_params['evalfile'] = self.evalfilestem.format(series=self.series,cfg=self.cfg)
    ...
    def set_contraction_type(self):
        ...
        self.npoint = int(re.match(".*_([0-9])pt.*",self.contraction_type).group(1))
    ...
    def qed_conn_4pt(self,contraction):
        ...
        matg1 = [self.load_meson(contraction[0],contraction[1],gamma) for gamma in gammas]
        matg2_photex = []
        matg2_selfen = []

        for gamma in gammas:
            if "photex" in self.subdiagrams:
                matg2_photex.append(self.load_meson(contraction[4],contraction[5],gamma))
            if "selfen" in self.subdiagrams:
                matg2_selfen.append(self.load_meson(contraction[6],contraction[7],gamma))
        ...
        for i in range(self.Nem):
            ...
            if "selfen" in self.subdiagrams:
                matp2_selfen = self.load_meson(contraction[4],contraction[5],emlabel)
            if "photex" in self.subdiagrams:
                matp2_photex = self.load_meson(contraction[6],contraction[7],emlabel)
...
def main():
    ...
    if len(sys.argv) != 2:
        logging.error("Must provide sub-ensemble and config data in 'series.ensemble' format")
        exit()
    ...
    for diagram,contractor in contractor_dict.items():
        ...
        outfile_replacements = formatdict(contractor.outfile,contractor.__dict__)

        overwrite_outfile = 'overwrite' not in contractor.__dict__ or contractor.overwrite

        nmesons = contractor.npoint

        low_range = range(0,nmesons+1) if contractor.has_high else [nmesons]
        low_range = [0] if not contractor.has_low else low_range

        for nlow in low_range:
    
            if contractor.has_high:
                seeds = list(itertools.combinations(list(range(contractor.high_count)),nmesons-nlow))
            else:
                seeds = [()]
            ...
            for perm in perms:
                ...
                for seed in seeds:
                    ...
                    for i in range(len(perm)):
                        j = (i+1) % len(perm)
                        if perm[i] == 'L':
                            contraction.append(contractor.low_label)
                        else:
                            contraction.append(f"w{contractor.high_label}{seed[i]:d}")
                        if perm[j] == 'L':
                            contraction.append(contractor.low_label)
                        else:
                            contraction.append(f"v{contractor.high_label}{seed[j]:d}")
                    contraction_list.append(contraction)

                start_time = perf_counter( )

                corr = dict(zip(map(contractor.make_contraction_key,contraction_list),
                             map(contractor.execute,contraction_list)))

                stop_time = perf_counter( )

                logging.debug('')
                logging.debug('    Total elapsed time for %s = %g seconds.' % (permkey,stop_time - start_time) )
                logging.debug('')
